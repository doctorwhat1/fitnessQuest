...
class SettingsFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        setting1EditText.setText(sharedPref.getInt("steps_daily",6000).toString())


        setting2EditText = view.findViewById(R.id.input_calories_daily)
        setting2EditText.setText(sharedPref.getInt("calories_daily",2500).toString())
        ...
        saveButton.setOnClickListener()
        {
            ...
            val steps_daily = setting1EditText.text.toString().toInt()
            if (steps_daily >= 3000) {
                editor.putInt(REQUIRED_TOTAL_STEPS, steps_daily)
            }
            //saveToSharedPref("steps_daily",steps_daily)


            val calories_daily = setting2EditText.text.toString().toInt()
            if (calories_daily >= 1000 && calories_daily <= 5000) {
                editor.putInt(REQUIRED_TOTAL_CALORIES, calories_daily)
            }
            editor.apply()
